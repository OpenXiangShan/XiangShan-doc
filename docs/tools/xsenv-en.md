...
Run `make verilog CONFIG=MinimalConfig` under `/xs-env/XiangShan`, this command will compile Xiangshan's Chisel code and generate Verilog, the output Verilog file is `XiangShan/build/XSTop.v`. 

### Generate dual core Verilog code

Run `make verilog CONFIG=MinimalConfig NUM_CORES=2` under `/xs-env/XiangShan`, this command will generate the Verilog of Xiangshan (dual core), the output Verilog file is `XiangShan/build/XSTop.v`

A simulation program of Xiangshan (minimal configuration) will be generated by the commands above, and `CONFIG=MinimalConfig` specifies the configuration used by the Xiangshan core. (See [Xiangshan parameter instruction](https://xiangshan-doc.readthedocs.io/zh_CN/latest/misc/config/)) We can delete the `CONFIG=MinimalConfig` above to generate Verilog code of complete Xiangshan core with default config.
...
src/checkpoint/serializer.cpp:54:28: error: ‘MEMORY_SIZE’ was not declared in this scope; did you mean ‘PMEM_SIZE’?
   54 |   const size_t PMEM_SIZE = MEMORY_SIZE;
      ...
      |                            PMEM_SIZE
```
> Please insert `extern unsigned long MEMORY_SIZE;` before that line
