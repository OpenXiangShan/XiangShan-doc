...
根据不同的条件，我们有多种多样的需要 Load 指令重发的情况，包括但不限于：
...
!!! note "奇怪的重发判断"
    ...
    但令人感慨的是，其实有的时候很多条件也是大家拍脑袋想出来的，可能只是一些经验性的假设，而不是经过严谨的科学分析后的结果。

    可能这也是敏捷开发的代价...?🧐😋


另一方面，为了使得非阻塞式的访存流水线可以充分利用，Load/Store Unit 还有一些关于入口请求来源的仲裁。这其中，LoadUnit 的入口来源仲裁最为“恶心”。
...
类似地，对于访存流水线的最后阶段，需要写回给 BackEnd。在这一点上，LoadUnit 与 StoreUnit 也展现出了极大的不同。
...
!!! note "另一种 Trade-Off 的体现"
    这其实可能也是一种 Trade-Off，事实上，无论如何，使用一个单独的 Replay Queue 来进行重发任务理论上都要比基于 Issue Queue 进行重发在性能上要更好。但是显然，如果要再设计一个 Store Replay Queue 的话要付出额外的硬件代价，如果和 Load 复用 Replay Queue 的话可能影响到 Load 的 Replay，并引入额外的控制逻辑。

    但无论如何，这也只是我们的设想，我们也欢迎有同学去尝试其他的 Store 重发的实现方式。


另外，StoreUnit 会负责进行 RAW 的检查，以及将当前流水线的信息传递给 LoadUnit 进行 st-ld 的违例检查。

具体地，Store 指令会去查询 LoadQueueRAW，并根据查询情况判断是否需要生成流水线刷新。同时，会在 StoreUnit S1 将相关信息传递给 LoadUnit 进行 st-ld 违例检查。
...
事实上，整个 LoadUnit 在整体的大的功能点上并不复杂，甚至可以完整地概括为两个功能点：**权限检查和获取数据**。但在微架构实现上，为了支持 LoadUnit 正确地完成这两项工作，我们却付出了非常大的代价。不仅仅是需要为 LoadUnit 建立一整套配套的模块，还不得不在 LoadUnit 中写出十分复杂的控制逻辑。
...
!!! note "不得不品鉴的代码"
    ...
    **在这里，你可以看到像 Verilog 一样的赋值连线，足足有近一千行（。**
...
前面我们提到，LoadUnit 需要发出唤醒信号来唤醒依赖 Load 的指令，但这并非是无条件的。在一些情况下，LoadUnit 是不会发出唤醒信号的。但大多数情况下，对于一条普通的标量 Load 指令，在进入 LoadUnit 的时候就会发出唤醒信号，而如果一切顺利，这条 Load 指令不会取消掉这次唤醒，会伴随着写回唤醒其他依赖于 TA 的指令。而如果产生了各种各样的不正常情况，例如需要 Replay，那么就需要发出 Cancel 信号，来通知 Issue Queue 取消这次唤醒。
...
而除了上面这些以外，LoadUnit 还需要进行一些违例检查的操作，主要是根据情况申请进入 RAR/RAW Queue，以便后续进行违例检查。还需要根据 StoreUnit 传过来的位于 StoreUnit S1 的信息进行 st-ld 的违例检查，并根据情况生成流水线刷新。
...
!!! note "其实可能也不会再展开了（"
    ...
    换言之，阅读入门指南并学习昆明湖的这段时间是你十分宝贵的时间，我们也可以说是新人保护期。如果我们认为你通过了新人保护期，或者你使用某些不太合适的方式快速地度过了新手保护期，那你应该明白，我们会认为你已经具备了独立解决某些问题的能力。所以，珍惜这段时间并努力学习吧。
