...
说到最后，让我们再来回顾一下我们的世界最大的停车场，经十路吧。

正如前文所述，为了缓解交通压力，经十路在过去数年间持续进行拓宽工程，但实际的通行效率并未得到根本性改善。究其原因，人们在多年的道路实践中逐渐意识到，**<font style="color:#AD1A2B;">决定整条道路吞吐量的关键，往往不在于主干道最宽处的容量，而受限于瓶颈路段的短板</font>**。即使主干道足够宽阔，与之相交的支路通行能力却参差不齐，当不同流量的道路在路口交汇时，必然会限制整体车流速度。
更为棘手的是，现实的交通环境极其复杂，道路参与者并非只有单一的机动车流。当非机动车与行人介入道路系统时，机动车在路口不得不频繁减速或停车避让。**<font style="color:#AD1A2B;">这种混合交通流带来的客观限制，导致单纯增加车道宽度无法解决拥堵问题</font>**。

**这一现象与计算机体系结构中向量单元的设计困境如出一辙。**

事实上，在向量运算单元中单纯的增加运算通道数量就等同于对道路进行“拓宽”。虽然理论上这能成倍提升数据吞吐量，但在实际应用中，单纯增加运算通道的数量并不总能带来线性的性能收益，原因恰恰对应了上述有关经十路不断扩宽道路却无法解决拥堵问题的的两个痛点：

一方面，现代处理器中的瓶颈往往是内存墙，我们虽然可以不断的增加我们的运算单元，但是我们却无法无限的从内存获取充足的数据，因此，在很大程度上，无论运算单元设计的有多少，其运算吞吐最终一定会因为处理器达到内存瓶颈而停下来。就像经十路最宽的地方即使再宽，也肯定会因为最窄的地方而造成拥堵一样。

另一方面，我们总是期望我们的向量处理器能够整齐规律的执行运算任务，尽可能减少其他逻辑的干扰，但事实上，在真实的任务中却很难做到这一点。现代处理器往往要处理很多复杂的控制逻辑，这会对我们的向量执行造成很大的干扰，即使 RVV 已经使用了 Mask 来尽可能将控制依赖转换为数据依赖，但是其他复杂的分支控制依赖依然是无法避免的。这也就和经十路上的机动车与非机动车的冲突一样。
